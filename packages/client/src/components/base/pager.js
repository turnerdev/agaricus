...
function handleNextPage(host, _) {
  if (host.currentPage < host.pages) {
    changePage(host, host.currentPage + 1)
  }
}

function handlePreviousPage(host, _) {
...
export default {
  ...
  currentPage: 0,
  displayedPages: {
    get: ({ currentPage, pages, neighbourhood }) => [
      ...new Set([
        1,
        ...[...Array(1 + neighbourhood * 2).keys()]
          .map(i => i + currentPage - neighbourhood)
          .filter(page => page >= 1 && page <= pages),
        pages,
      ]),
    ],
  },
  render: ({ currentPage, displayedPages }) =>
    html`
      <nav
      ...
      >
        ...
        ${displayedPages.map(
          (page, i) => html`
            ${i > 0 &&
            displayedPages[i - 1] !== page - 1 &&
            html` <div class=${classes.ellipsis}>...</div> `}
            <a
              href="javascript:void(0)"
              data-page="${page}"
